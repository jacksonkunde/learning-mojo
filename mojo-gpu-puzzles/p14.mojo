...
fn prefix_sum_simple[
    ...
    var hop_size: Int = 1
    while hop_size < TPB:
        if local_i - hop_size >= 0:
            shared[local_i] = shared[local_i] + shared[local_i - hop_size]
        hop_size *= 2
        barrier() # sync
    
    # shared is now our valid result
    if global_i < size:
...
fn prefix_sum_block_sum_phase[
    ...
    if block_idx.x > 0 and global_i < size:
        # add the result of the previous block thread 
        output[global_i] += output[block_dim.x * block_idx.x - 1]
