...
fn naive_matmul[
    ...
    if row < size and col < size:
        var dot_prod: output.element_type = 0
        @parameter
        for k in range(size):
            dot_prod += a[row, k] * b[k, col]

        output[row, col] = dot_prod
...
fn single_block_matmul[
    ...
    # load matricies into shared memory
    if row < size and col < size:
    ...
    if row < size and col < size:
        ...
        @parameter
        for k in range(size):
            dot_prod += a_shared[local_row, k] * b_shared[k, local_col]
