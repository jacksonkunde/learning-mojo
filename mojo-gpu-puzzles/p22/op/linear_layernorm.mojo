...
fn minimal_fused_kernel[
    ...
    # Compute the mean and var over the hidden dim
    @parameter
    ...
    for od in range(output_dim):
        var hd_sum: Scalar[dtype] = 0
        @parameter
        for hd in range(hidden_dim):
            # Divide each elet by the variance + \eps
            var elet = rebind[Scalar[dtype]](input[batch_idx, seq_idx, hd])
            hd_sum += ((elet - mean_val) * inv_std * rebind[Scalar[dtype]](ln_weight[hd]) + rebind[Scalar[dtype]](ln_bias[hd])) * rebind[Scalar[dtype]](linear_weight[od, hd])

        output[batch_idx, seq_idx, od] = hd_sum + linear_bias[od]
